...
// QueryKits chains the current query on the "kits" edge.
func (_q *UserQuery) QueryKits() *BrandKitQuery {
	query := (&BrandKitClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithKits tells the query-builder to eager-load the nodes that are connected to
// the "kits" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithKits(opts ...func(*BrandKitQuery)) *UserQuery {
	query := (&BrandKitClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withKits; query != nil {
		if err := _q.loadKits(ctx, query, nodes,
			func(n *User) { n.Edges.Kits = []*BrandKit{} },
			func(n *User, e *BrandKit) { n.Edges.Kits = append(n.Edges.Kits, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadProviders(ctx context.Context, query *ProviderQuery, nodes []*User, init func(*User), assign func(*User, *Provider)) error {
	...
	for _, n := range neighbors {
		fk := n.user_providers
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_providers" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_providers" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadKits(ctx context.Context, query *BrandKitQuery, nodes []*User, init func(*User), assign func(*User, *BrandKit)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.BrandKit(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.KitsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_kits
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_kits" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_kits" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
